#' calculate the price of European option by diffrence method
#'
...
#'
#' @examples
#' Eoption(30,0.1,1,0.1,0.1,1,"c")
Eoption <- function(D,sigma,S0,r,q,K,type){
  K <- K/S0
  ...
  for (n in N:1) {
    for (m in (N+2-n):(n+N)) {
      V[m,n] <- (k1*V[m,n+1]+k2*V[m+1,n+1]+k3*V[m-1,n+1])/k0
    }
  }
  #output
  V0 <- S0*V[N+1,1]
